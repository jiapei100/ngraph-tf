...
void BuilderTestSimple::ExecuteOnNGraph() {
  ...
  NGRAPH_VLOG(5) << "Found graph data , found op "<< test_op->type_string();
  ...
  //ExecuteOnNGraph(ng_function, input_tensors, );
  ...
  //int number_of_inputs = tf_inputs_.size();
  //int number_of_outputs = ngraph_outputs_.size();
...
void BuilderTestSimple::ComputeOnNGraph(Graph& graph, string test_op_type,
